...
form.onsubmit = async e => {
  ...
  if (jobs || cityInfo) {
    ...
    <p style="text-align: center">Try searching again.</p>`;
  }
};
...
function renderCityInfo({ scores, overall_score: overallScore }) {
  const getBgColor = color =>
    parseInt(color.slice(1, 3), 16) * 0.299 +
    parseInt(color.slice(3, 5), 16) * 0.587 +
    parseInt(color.slice(5, 7), 16) * 0.114 > 110
      ? '#00000000'
      : '#fff';
  ...
  return `<section>
    ...
    <div class="scores">
      ${scores.map(({ name, color, score_out_of_10: score }) => `
    ...
    <p class="overall-score">Overall Score: ${overallScore.toFixed(0)}/100</p>
  </section>`;
}
...
function renderJobs(jobsArr) {
  return `<section>
    <h3>Job Listings</h3>
    ${jobsArr.map(({ company_name: name, role, company_num_employees: numEmployees, location, url, text: description, date_posted: posted }) => `
      <div class="listing">
        <h4>Company: ${name}</h4>
        <p>Role: ${role}</p>
